...
struct Home: View {
    ...
    var body: some View {
        ZStack {
            ...
            .rotation3DEffect(
                Angle(degrees:showProfile ? Double(vState.height/10)-10 : 0),
            ...
            MenuView()
                ...
                .offset(y: vState.height)
                .animation(.spring(response: 0.5, dampingFraction: 0.6, blendDuration: 0))
                .onTapGesture {
                    self.showProfile.toggle()
                }
                .gesture(DragGesture().onChanged {
                    value in self.vState=value.translation
                    if self.vState.height > 50 {
                        self.showProfile = false
                    }
                }
                .onEnded {
                    value in self.vState = .zero
                })
            if showContent {
                Color.white
                    .edgesIgnoringSafeArea(.all)
